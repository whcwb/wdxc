...
import com.ldz.obd.common.tools.SpringContextUtil;
import com.ldz.obd.handler.biz.BizBaseHandler;
import com.ldz.obd.handler.biz.BizHandlerDefault;
...
public class ServerChannelHandler extends ChannelInboundHandlerAdapter{
	...
	//读取系统参数命令handler
	@Autowired
	private BizHandlerDefault bizHandlerDefault;
	//限速器
	@Autowired
	...
	public void channelRead(ChannelHandlerContext ctx, Object msg) {
		try {
			...
			String handlerName = "bizHandler" + messageBean.getCommandType();
			BizBaseHandler handler;
			if (SpringContextUtil.containBean(handlerName)){
                handler = (BizBaseHandler) SpringContextUtil.getBean(handlerName);
            }else{
                handler = bizHandlerDefault;
            }
	 ...
	 * @param ctx
	 */
	private void writeData(ChannelHandlerContext ctx, MessageBean sendData){
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) {
		if (cause instanceof BizException){
			...
			String deviceId = ctx.channel().attr(ServerChannelHandler.DEVICENO).get();
			log.error("通道["+deviceId+"]连接超时");
			stringRedis.convertAndSend("connect_timeout",deviceId);
		}else{
