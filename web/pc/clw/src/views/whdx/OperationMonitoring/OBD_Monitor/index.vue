...
<template>
      <div class="box-row">
            ...
            <div style="position:absolute;width:350px;top:0;right:0;z-index:9990;padding-top:30px;padding-right:30px;float: right"
...
<script>
    ...
    export default {
        ...
        methods: {
            startQuery(){
                clearTimeout();
                if (this.$route.path != '/OperationMonitoring/OBD_Monitor'){
                    return;
                }
                let v = this;
                setTimeout(()=>{
                    if (v.$route.path != '/OperationMonitoring/OBD_Monitor'){
                        return;
                    }
                    v.queryGps()
                    v.startQuery();
            ...
            subscribe() {
                ...
                for (let r of showCarList) {
                    ...
                    // });
                    this.subscribes[r.zdbh] = this.websocketUtil.subscribe('/topic/check-' + r.zdbh, function (data) { //订阅消息
            ...
            initGps() {//获取取终端
                var v = this
                this.$http.get(this.apis.CLJK.QUERY,{params:{zdLx:'20'}}).then((res) => {
                    if (res.code === 200) {
