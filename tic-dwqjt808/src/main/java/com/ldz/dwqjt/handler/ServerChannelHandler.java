...
import com.ldz.dwqjt.handler.biz.BizBaseHandler;
...
public class ServerChannelHandler extends ChannelInboundHandlerAdapter{
	...
	public final static AttributeKey<Integer> STATUS = AttributeKey.newInstance("status");
	public final static AttributeKey<Integer> FLOWID = AttributeKey.newInstance("flowid");
	//同步执行属性
	...
	public final static AttributeKey<LinkedBlockingQueue<PackageData>> CMDQUEUE = AttributeKey.newInstance("queue");
	@Autowired
	...
	public void channelRead(ChannelHandlerContext ctx, Object msg) {
		try{
			...
			accessLog.info("新消息："+packageData.toString());
			...
			String handlerName = "bizHandler" + header.getMsgId();
			// 根据命令类型判断是否有相应的处理器
			if (!SpringContextUtil.containBean(handlerName)){
				log.error("未知命令类型："+header.getMsgId());
				return;
			}
			//接收命令成功，更新终端在线状态
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) {
		if (cause instanceof RuntimeCheckException){
			return;
